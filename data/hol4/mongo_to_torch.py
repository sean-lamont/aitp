...
class HOL4DataModule(LightningDataModule):
    ...
    def prepare_data(self) -> None:
        if not os.path.exists(self.dir):
            ...
            train_data = [self.to_relation((graph_dict[v['conj']], graph_dict[v['stmt']],v['y'])) for v in split.find({}) if
                          v['split'] == 'train']
            val_data =  [self.to_relation((graph_dict[v['conj']], graph_dict[v['stmt']], v['y'])) for v in split.find({}) if
                          v['split'] == 'val']
            test_data = [self.to_relation((graph_dict[v['conj']], graph_dict[v['stmt']], v['y'])) for v in split.find({}) if
                        v['split'] == 'test']
    ...
    def collate_pad(self, batch):
        ...
        return Data(xi=xi1, xj=xj1, edge_attr_=edge_attr_1, mask=mask1),Data(xi=xi2, xj=xj2, edge_attr_=edge_attr_2, mask=mask2),torch.LongTensor(ys)
