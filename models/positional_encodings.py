...
def get_magnetic_Laplacian(edge_index: torch.LongTensor, edge_weight: Optional[torch.Tensor] = None,
    ...
    if not return_eig:
        ...
        k = min(k, L.shape[0])
        # print (k, L.shape[0])
        eig_vals, eig_vecs = eigsh(L, k=k - 2, which='LM', return_eigenvectors=True)
...
class MagLapNet(torch.nn.Module):
    ...
    def forward(self, eigenvalues,
        ...
        eigenvalues_ = einops.repeat(eigenvalues, "k -> n k", n=trans.shape[-1])
