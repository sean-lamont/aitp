...
class AttentionRelations(nn.Module):
    ...
    def forward(self,
        ...
        xi = self.embedding(xi) # if hasattr(batch, "node_depth") is None else self.embedding(x, batch.node_depth.view(-1,))
        xj = self.embedding(xj) # if hasattr(batch, "node_depth") is None else self.embedding(x, batch.node_depth.view(-1,))
