...
def tactic_zero_experiment(config):
    ...
    # logging.basicConfig(filename=config.exp_config.directory + '/log', level=logging.DEBUG)
    ...
    arg_net = ArgPolicy(len(config.tactic_config.tactic_pool), config.model_config.embedding_dim)
    term_net = TermPolicy(len(config.tactic_config.tactic_pool), config.model_config.embedding_dim)

    induct_net = FormulaNetEdges(config.model_config.vocab_size, config.model_config.embedding_dim,
